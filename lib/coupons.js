...
function generateCouponCode() {
  return `CJ-${randomSegment(4)}-${randomSegment(4)}`;
}
...
function normalizeCouponCode(input) {
  ...
  if (cleaned.startsWith('CJ')) {
    ...
    if (rest.length === 8) return `CJ-${rest.slice(0, 4)}-${rest.slice(4)}`;
  }
  if (cleaned.length === 8) return `CJ-${cleaned.slice(0, 4)}-${cleaned.slice(4)}`;
  return null;
}
